...
class Download extends EventEmitter{
  constructor(url, fix='', {std=process.stderr, proxy='localhost:1087', type='ts'} = {}) {
    super()
    Object.assign(this, {
      url, fix, std, proxy, type,
      pbar: null, chunks: [], begin: new Date(),
      name:`${basename(url, '.m3u8')}.${type}`,
    })
  ...
  spawn () {
    ...
    switch (type) {
      case 'ts':
        args = ['-y', '-i', this.url, '-c', 'copy', this.name]
        break
      case 'mp4':
        args = ['-y', '-i', this.url, '-c', 'copy', '-bsf:a', 'aac_adtstoasc', this.name]
        break
    }
